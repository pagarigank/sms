...
import { Plus, Building, GraduationCap, Check, MoreHorizontal, Search } from 'lucide-react';
...
import { Trash2, Edit, PauseCircle, PlayCircle } from 'lucide-react';
...
export default function DepartmentsPage() {
  ...
  const renderNameCell = ({ row }: { row: { original: Department } }) => (
  ...
  const renderTenantCell = ({ row }: { row: { original: Department } }) => (
    <div>
  ...
  const renderBranchCell = ({ row }: { row: { original: Department } }) => (
    <div>
  ...
  const renderEducationLevelsCell = ({ row }: { row: { original: Department } }) => {
    const levels = row.original.educationLevelIds
      .map(id => educationLevels.find(el => el.id === id)?.name)
      .filter(Boolean)
      .join(', ');
    return levels || '—';
  };

  const renderDefaultCell = ({ row }: { row: { original: Department } }) =>
  ...
  const renderCreatedCell = ({ row }: { row: { original: Department } }) =>
    row.original.createdAt ? new Date(row.original.createdAt).toLocaleDateString() : '—';

  const renderActionsCell = ({ row }: { row: { original: Department } }) => {
  ...
  const columns: ColumnDef<any, any>[] = [
    ...
    { accessorKey: 'tenantName', header: 'Tenant', cell: renderTenantCell },
    { accessorKey: 'branchName', header: 'Branch', cell: renderBranchCell },
    { accessorKey: 'educationLevelIds', header: 'Education Levels', cell: renderEducationLevelsCell },
    { accessorKey: 'isDefault', header: 'Default', cell: renderDefaultCell },
