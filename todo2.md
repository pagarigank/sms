...
- [ ] **[GAP-2/G-16 — MIDDLE]** RLS real-drill verification: connect as `sms_app` (non-owner, NOBYPASSRLS), confirm `SET LOCAL app.current_tenant_id` is honored per the fixed `TenantAwareDataSource` path, and prove a cross-tenant read returns 0 rows. Prerequisite G-15 (api-client `x-tenant-id`) is already wired in `ApiClient` (tenantId from auth store → header on every request). Once the drill passes, mark G-16 closed.
...
- **Typecheck:** backend (`tsconfig.build.json`), api-client, school-portal, platform-admin, guardian-portal — all pass. G-16 live RLS drill remains the only pre-existing open blocker (unchanged, requires a live DB drill).
...
- **[MIDDLE] G-16 (RLS real-drill verification):** The GUC path is fixed and exercised per-request via `TenantAwareDataSource`, but the live drill (connect as non-owner `sms_app`, confirm `SET LOCAL` is honored, prove cross-tenant reads return 0 rows) has not been run on a real database. This is the only item that blocks the Phase 1 exit claim "RLS enforces cross-tenant isolation" from being verifiable rather than assumed.
